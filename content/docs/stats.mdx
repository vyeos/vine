...
```
GET https://api.vinecms.online/api/public/v1/{API_KEY}/stats
```

## Example Request

```bash
curl "https://api.vinecms.online/api/public/v1/{API_KEY}/stats"
```
...
const response = await fetch(
  'https://api.vinecms.online/api/public/{API_KEY}/stats'
);
...
function WorkspaceStats() {
  ...
  useEffect(() => {
    fetch('https://api.vinecms.online/api/public/{API_KEY}/stats')
      .then(res => res.json())
      .then(setStats);
  }, []);
