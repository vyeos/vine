...
function SettingsHub({
  ...
  const isSavingAccount =
    editProfileMutation.isPending || updateAvatarMutation.isPending;
      ...
      <div className="grid gap-8 lg:grid-cols-[200px_minmax(0,1fr)] lg:items-start">
        ...
        <div className="flex flex-col gap-6">
          ...
          >
            <div className="flex flex-col gap-4">
              <div className="flex items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  <Avatar className="size-10 rounded-lg border">
                    <AvatarImage
                      src={overview?.user.avatar ?? user.avatar}
                      alt={user.name}
                    />
              ...
              <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-muted-foreground">
                <span>
                  {overview?.summary.workspaceCount ?? 0} workspace
                  {(overview?.summary.workspaceCount ?? 0) !== 1 ? "s" : ""}
                </span>
                ...
                <span className="text-border">|</span>
                <span>
                  {overview?.summary.memberWorkspaceCount ?? 0} member
                </span>
                {overview && (
                  <>
                    <span className="text-border">|</span>
                    <span>
                      Joined {formatDate(overview.summary.memberSince)}
                    </span>
                  </>
                )}
              </div>
                ...
                overview.memberships.map((membership) => (
                  ...
                  >
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={() =>
                        router.push(getWorkspacePath(membership.slug))
                      }
                    >
                      Open
          ...
          <SettingsSection
            id="preferences"
            title="Notifications"
            description="Control the alerts and emails you receive."
          >
            <div className="flex flex-col gap-4">
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              <Avatar className="size-12 rounded-lg border">
                <AvatarImage
                  src={previewAvatar}
                  alt={overview?.user.name ?? user.name}
                />
            ...
            <div className="space-y-1.5">
              ...
              <Input
                id="profile-email"
                value={overview?.user.email ?? user.email}
                disabled
              />
            </div>
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-1.5">
                ...
                <Select
                  value={avatarMode}
                  onValueChange={(value) =>
                    setAvatarMode(value as "provider" | "custom")
                  }
                >
